...
app.post('/login', (req, res) => {
    ...
    async (err, results) => {
        if(err) return res.status(500)._construct.send('Server com erro');
        if(results.lenght == 0) return res.status(500).send('CPF ou senha incorretas')

        const usuario = results[0];
        const senhaCorreta = await bcrytp.compare(senha, usuario.senha);
        if(!senhaCorreta) return res.status(500).send(
            'CPF ou senha incorretos');

        req.session.useId = usuario.idUsuarios;
        console.log('idUsuario')
        res.json({message: 'Login bem-sucessido'});
    });
})
